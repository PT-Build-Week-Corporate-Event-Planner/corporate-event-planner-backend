...
const verifyAndCleanTask = ( taskName, taskCompleted, eventId ) => {
  ...
  if( !taskName ){
    return {
      task_completed: taskCompleted, event_id: eventId,
    };
  }
  
  if( !eventId ){
    return {
      task_completed: taskCompleted, task_name: taskName,
    };
  }
  
  if( !notUndefined( taskCompleted ) ){
    return {
      task_name: taskName, event_id: eventId,
    };
  }
  
  return {
    task_name: taskName, task_completed: taskCompleted, event_id: eventId,
  };
};
